...
class RemoveCacheCommandTest extends AbstractCommandTestCase
{
    ...
    public function testShouldRemoveGlobsWithCacheExists()
    {
        $images = array('images/cats.jpeg', 'images/cats2.jpeg');
        ...
        $this->assertImagesNotExist($images, $filters);
        $this->assertOutputContainsRemovedGlobImages($output, $filters);
        $this->assertOutputContainsGlobSummary($output, $filters);
    ...
    /**
     * @return RemoveCacheCommand|object
     */
    private function getRemoveCacheCommand(): RemoveCacheCommand
    {
        return $this->getService('liip_imagine.command.cache_remove');
    }
     ...
     * @param array  $filters
     */
    protected function assertOutputContainsRemovedGlobImages($output, array $filters)
    {
        foreach ($filters as $f) {
            $this->assertContains(sprintf('*[%s] (removed-glob)', $f), $output);
        }
    }
     ...
     * @param int      $failures
     */
    protected function assertOutputContainsGlobSummary($output, array $filters, $failures = 0)
    {
        $this->assertContains(sprintf('Completed %d removal', count($filters) - $failures), $output);
